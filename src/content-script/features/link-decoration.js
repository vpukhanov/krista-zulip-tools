...
const isMessageHref = (href, realm) =>
    typeof href === 'string' && href.startsWith(realm) && messageIdRegex.test(href)
...
const decorateLink = (link, zulip) => {
    ...
    zulip.messages.retrieve({ anchor: id, num_before: 0, num_after: 0 })
        .then((result) => {
...
const decorateLinks = (links, zulip, realm) =>
    links
        .filter((link) => !link.getAttribute('data-zkt') && isMessageHref(link.href, realm))
...
export default (zulip, configuration) => {
    ...
    new MutationObserver((mutations) =>
        mutations
            ...
            .map(({ target }) => decorateLinks(Array.from(target.getElementsByTagName('a')), zulip, realm))
